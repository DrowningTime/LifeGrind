...
func _ready():
	...
	var area_menu = _create_area_menu(area_menu_name, area_button)
...
func _create_area_button(label, area_name, area_label):
	...
	var button_position = area_label.global_position + Vector2((area_label.get_size().x - button.get_size().x) / 2, area_label.get_size().y)
